...
| `--color-accent-orange`        | `#e8872e` | Модуль «Ремонт»: вкладки, итоги, точки «отклонение»   |
...
Типографика — шкала `--font-size-3xs … 2xl`:
`3xs: 0.6875rem` (11px), `2xs: 0.7rem`, `xs: 0.8rem`, `caption: 0.75rem` (12px), `sm2: 0.85rem`,
`sm: 0.9rem`, `md: 1rem`, `lg: 1.2rem`, `xl: 1.8rem`, `2xl: 2.6rem`. Применена в примитивах, заголовках
и всём тексте страниц (Фаза 6/A3). Осознанно «сырыми» остаются только responsive-размеры заголовков
в `@media` (`2rem`/`1.6rem`/`1.4rem`/`1.5rem`) и `16px` у полей форм (анти-зум iOS).
...
- ✅ **Фаза 6 (A3).** Токенизация шкал: радиусы (все значения — в `--radius-*`, пилюля `--radius-pill`),
  типографика (сырые размеры — в `--font-size-*`, добавлены `3xs`/`caption`/`sm2`), отступы (точные
