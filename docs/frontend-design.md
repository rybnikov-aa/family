...
API-контрактов и UI-поведения (все фазы реализованы — см. «Статус фаз» в конце документа).
...
| `--color-accent-blue`          | `#3b82f6` | «Дневник»: иконка страницы, pill-даты, обложка, карточки  |
...
| `--space-icon-gap` | `6px` — зазор «иконка–текст» в инлайн-ссылке/кнопке (`.btn`, `.renov-link`, `.renov-meta__link`)                                                |

Применены в примитивах (`btn`, `icon-btn`, `modal`, `input`, `alert`, `badge`) и основных оболочках
(`.container`, `.card`, `.modal__vps`, `.modal__service`, `.services-editor`). Именованный шаг
`--space-icon-gap: 6px` (между 4 и 8) — для повторяющегося паттерна «иконка перед текстом».
...
CSS: `.tooltip-trigger`, `.tooltip` — в `styles/tooltip.css` (глобальный слой, не привязан к модалкам);
слой — токен `--layer-tooltip`.

### `DiaryPhotoUploadActions` (`components/DiaryPhotoUploadActions.tsx`)

Общий набор кнопок добавления фото события «Дневника»: с диска (`ImageUpIcon`) и из Immich
(`ImagePlusIcon`). Используется в форме события, модалке фотографий и редакторе описания.

| Проп              | Тип                       | Описание                               |
| ----------------- | ------------------------- | -------------------------------------- |
| `immichAvailable` | `boolean`                 | Показывать кнопку добавления из Immich |
| `onAddFiles`      | `(files: File[]) => void` | Передать выбранные файлы               |
| `onOpenImmich`    | `() => void`              | Открыть пикер Immich                   |

Загрузка с диска — через скрытый `<input type="file">` внутри компонента.
...
`margin: auto` (а не `justify-content: center`), поэтому при увеличении масштаба переполнение
остаётся доступным прокруткой в обе стороны. Ошибки рендера страницы (например, превышение
лимита размера canvas на некоторых устройствах) показываются вместо пустой «белой» страницы.

Совместимость: pdf.js v6 использует `Map.prototype.getOrInsertComputed` (ES2025), которого нет в
Samsung Internet и части старых WebView — без него рендер падает с
`this[#t].getOrInsertComputed is not a function`. В модуле `utils/pdfPolyfills.ts` (`installPdfPolyfills`)
установлен полифилл по спецификации; вызывается первым делом в `PdfViewerModal.tsx` до любого
использования pdf.js. При обновлении `pdfjs-dist` проверять, не нужны ли полифиллы новых API.

### Ссылка на PDF (`components/PdfLink.tsx`)

Ссылка-кнопка на исходный PDF документа «Ремонта» (дизайн-правило: если по ссылке открывается
просмотр PDF, перед такой ссылкой всегда стоит иконка документа `DocIcon`). Открывает документ
во встроенном просмотрщике (`onOpenPdf`).

| Проп         | Тип                                 | По умолчанию | Описание                                                             |
| ------------ | ----------------------------------- | ------------ | -------------------------------------------------------------------- |
| `url`        | `string`                            | —            | URL исходного PDF                                                    |
| `title`      | `string`                            | —            | Заголовок просмотрщика PDF                                           |
| `onOpenPdf`  | `(url, title, fitToWidth?) => void` | —            | Открыть PDF в просмотрщике (см. `PdfViewerModal`)                    |
| `fitToWidth` | `boolean`                           | `false`      | Растягивать форму под ширину документа (дизайн-проект)               |
| `children`   | `ReactNode`                         | —            | Текст ссылки (имя документа)                                         |
| `className`  | `string`                            | —            | Доп. класс (например `design-modal__link` для списка дизайн-проекта) |

CSS: `.renov-link` (+ иконка `DocIcon` 16px перед текстом — `.renov-link svg`).
Используется в списках документов карточек-сводок («Работы»/«Материалы», `RenovDocRow`),
заголовках ведомостей взаиморасчётов (`RenovationSettlement`), шапках заказов отчёта
«Материалы» (`RenovationMaterialsReport`) и модалках «Смета»/«Дизайн-проект»
(`RenovationEstimateModal`/`RenovationDesignModal`).

### Успешный результат модалки (`components/ModalDone.tsx`)

Блок «готово» после успешного действия модалки: зелёная галочка, сообщение и кнопка
«Закрыть». Общий примитив вместо дублей `.addendum__done`/`.renov-pdf__done`
(модалки «Доп. соглашение» и «Импорт PDF»).

| Проп      | Тип          | По умолчанию | Описание                  |
| --------- | ------------ | ------------ | ------------------------- |
| `message` | `ReactNode`  | —            | Текст результата          |
| `onClose` | `() => void` | —            | Действие кнопки «Закрыть» |

CSS: `.modal-done`, `.modal-done__icon`, `.modal-done__actions`.
...
9. Ссылки на исходные PDF — только через `PdfLink`: иконка документа `DocIcon` перед текстом
   обязательна. Ссылка, открывающая просмотр PDF, без иконки документа не допускается.
...
- ✅ **Фаза D (примитивы и консолидация).** Компоненты `StatRow`/`RenovationSummaryCard`/`RenovationSettlement`
  ...
  консолидирована в 7 шагов (`3xs`/`xs`/`sm2` → `2xs`/`caption`; вариант 3); контраст активной кнопки-иконки
  (`.icon-btn--active`) — `--color-paper` вместо хардкода `#fff`
...
**Локальные тестовые учётки (намеренно сохранены для проверок UI):**
