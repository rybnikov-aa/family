...
- **Черновик** — `services/renovation/import/draft.ts`: разбор позиций/строк (таблицы pdfplumber,
  ...
  сумма» разбирается по токенам (`parseNumFields`), а не жадным regex с `[\d\s.,]+`: при
  пробеле-разделителе тысяч в одном из полей («91,00 60,00 5 460,00») выбирается вариант, где
  цена×кол-во = сумма (иначе жадный regex неверно склеивал поля). Наименование многострочной
  позиции, идущее до числовой строки (после шапки таблицы), копится в `pending` и подставляется
  в позицию. Черновики живут в памяти (`draftStore.ts`, TTL 30 мин).
...
- **«Ход работ»** — `services/renovation/reports.ts::buildWorkReport()` + `GET /api/renovation/reports/work`:
  ...
  нет); отклонение `факт − план` (`diff`) на бэкенде определяет статус и отдельной колонкой в UI
  не выводится. Секции сохраняют порядок сметы; `asOf` —
