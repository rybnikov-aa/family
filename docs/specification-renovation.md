...
- **Черновик** — `services/renovation/import/draft.ts`: разбор позиций/строк (таблицы pdfplumber,
  ...
  последней позиции, если она идёт сразу после её числовой строки или за ней не следует числовая
  строка; если же строка без номера стоит после имени другой позиции и за ней идёт числовая
  строка — это начало имени следующей позиции (накапливается в `pending`, не склеивается с
  предыдущим именем). Позициям присваивается раздел документа: строка «Раздел N. …» (в тексте — отдельной
