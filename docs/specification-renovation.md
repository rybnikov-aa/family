...
- **Реквизиты в шапке (`renov-meta`):** две карточки в одну строку на всю ширину (адаптивно для
  ...
  по клику на имя) | сумма, без отдельной кнопки «Открыть PDF»; блок «История» не выводится;
  для admin — кнопка «Доп. соглашение» → `AddendumModal`).
...
- **Строки документов в сводке** (акты в «Блоке 1. Работы», заказы в «Блоке 2. Материалы») —
  ...
  ведомости (`pdfPath` из сводки, компонент `components/RenovationSettlement.tsx`);
