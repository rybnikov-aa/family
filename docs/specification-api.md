...
`GET` — любому авторизованному (адрес используется для ссылок «Фотоархив»/«Архив» в UI);
`POST check` — только роль `admin` (иначе — 403). Подробно — `docs/immich.md`.
...
### 2.8. Immich: пикер фото (вариант B2)
...
в БД и клиенту не отдаётся. Все эндпоинты — только роль `admin` (роутер под `requireAdmin`).
Подробно — `docs/immich.md` (шаг 2).

| Метод | Путь                               | Назначение                                      | Параметры/ответ                                                                                                                                                                                                                                                                                                  |
| ----- | ---------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET   | `/api/immich/search`               | Поиск фото по диапазону дат съёмки (для пикера) | `takenAfter`/`takenBefore` (ISO datetime с часовым поясом — Immich требует offset, иначе 400), `page` (1-based), `size` (до 200); ответ — `{items: [{id, fileName, mimeType, takenAt}], total, nextPage}`; только изображения, допустимые к импорту (JPG/PNG/WebP/GIF); 400 — Immich не настроен, 401/403 — ключ |
| GET   | `/api/immich/assets/:id/thumbnail` | Миниатюра ассета (для сетки пикера)             | `id` — UUID ассета; ответ — изображение (прокси из Immich, `size=thumbnail`); 400 — некорректный id, 404 — не найден, 502 — ошибка апстрима                                                                                                                                                                      |
| GET   | `/api/immich/assets/:id/original`  | Оригинал ассета (импорт в событие) — стрим      | `id` — UUID ассета; ответ — файл (прокси потоком, `Content-Length` апстрима прокидывается — для прогресс-бара скачивания); 400 — некорректный id, 404 — не найден, 502 — ошибка апстрима                                                                                                                         |
...
содержит `pdfPath` — URL исходного PDF ведомости для ссылки-просмотра, `foremenAmount` —
сумма «подотчётные прораба» в ведомости, копейки, `null` если нет, и `note` — сноска
о коррекции данных (например, об ошибке в дате документа в исходной ведомости), `null` если
нет), `materialsBudget`
