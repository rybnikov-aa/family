...
| Метод | Путь                               | Назначение                                      | Параметры/ответ                                                                                                                                                                                                                                                             |
| ----- | ---------------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET   | `/api/immich/search`               | Поиск фото по диапазону дат съёмки (для пикера) | `takenAfter`/`takenBefore` (ISO datetime, необязательно), `page` (1-based), `size` (до 200); ответ — `{items: [{id, fileName, mimeType, takenAt}], total, nextPage}`; только изображения, допустимые к импорту (JPG/PNG/WebP/GIF); 400 — Immich не настроен, 401/403 — ключ |
