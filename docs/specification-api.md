...
Чтение — под `requireAuth`; мутации (создание/изменение/удаление) — под `requireAdmin`.
...
| POST   | `/api/diary` (admin)              | Создание события                             | multipart: `title`, `dateStart`, `dateEnd?`, `summary`, `content`, `images` (файлы, поле `images`), `newIds` (JSON-массив клиентских id), `keep` (JSON-массив, пустой), `cover` (id нового файла); 201 — `DiaryEventDetail`; 400 — невалидно; 413 — файл > 10 МБ; 400 — не-изображение / > 30 файлов |
...
| GET    | `/api/diary/images/:folder/:file` | Изображение события из `images/<folder>/`    | `folder`/`file` — имена (`[a-z0-9._-]`, без выхода за каталог); ответ — файл изображения; 400 — некорректный путь (path traversal), 404 — не найдено                                                                                                                                                 |
