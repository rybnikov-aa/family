...
Мутирующие операции (`POST /api/vps`, `POST /api/vps/import`, `DELETE /api/vps/:name`, `POST /api/projects`, а также все эндпоинты `/api/auth/admin/*`, мутации `/api/diary` и `POST /api/settings/immich/check`) доступны только роли `admin`; иначе — **403**. Чтение `/api/settings/immich` — любому авторизованному (адрес нужен для ссылок «Фотоархив»/«Архив» в UI; ключ не возвращается).
