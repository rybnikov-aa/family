...
| POST  | `/api/renovation/pdf`                       | Импорт PDF → черновик (admin)                             | multipart: `name` (имя файла), `file` (PDF); ответ — `{draft}` (201) или 422 (не удалось извлечь); PDF сохраняется как pending, при подтверждении переносится в `docs/renovation/` и пишется `pdf_path` |
...
| PUT   | `/api/renovation/meta`                      | Обновить адрес объекта «Ремонта» (admin)                  | тело — `{object: string}` (непустая строка, обрамляющие пробелы обрезаются); ответ — `{meta: RenovationMeta}`; 400 — пустой адрес; 403 — не admin                                                       |
