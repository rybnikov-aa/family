...
Подробно: [README.md](README.md) (структура, .env, деплой) · [docs/specification.md](docs/specification.md) (общая спецификация + модульные `specification-{vps,projects,auth}.md`) · [docs/server.md](docs/server.md) (nginx, SSL, сервер, диагностика).

## Правила

1. **Документация синхронно с кодом — обязательный чек-лист перед завершением.** При изменении кода (требования, конфиг, API, UI) обновлять **все**: модульные спецификации `docs/specification-*.md` (первой — соответствующая модульная спецификация; общий `docs/specification.md` — при изменении общих положений), затем `docs/server.md`, `README.md` и соответствующие `.env.example`. Для статичных страниц проектов (`projects/**`) при изменении макета/структуры — синхронно обновлять соответствующий навык (`.github/skills/*/SKILL.md`) и `docs/specification-projects.md`. Код корректен, если удовлетворяет критериям приёмки из модульной спецификации. Перед завершением задачи обязательно выполни:
   ...
   - Не ждать отдельной команды «обнови/актуализируй документацию»: правки `docs/`, навыков
     (`.github/skills/*/SKILL.md`) и `.env.example` входят в ту же задачу, что и код, — сразу после
...
10. **UI-итерации и позиционирование.** При правках отступов/выравнивания/порядка/размеров элементов:
    ...
    - после каждой правки позиционирования проверять фактический результат в браузере (обновить
