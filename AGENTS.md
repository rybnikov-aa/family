...
- **Локальная память `/memories/` — только удобное зеркало**: она не заменяет и не отменяет записи в репозитории. При завершении задачи важные выводы из локальной памяти синхронизируются в `.github/harness/repo-memory.md`.
