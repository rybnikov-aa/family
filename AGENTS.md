...
Подробно: [README.md](README.md) (структура, .env, деплой) · [docs/specification.md](docs/specification.md) (общая спецификация + модульные `specification-{api,vps,projects,auth,renovation,diary}.md`) · [docs/adr.md](docs/adr.md) (решения и обоснования ADR-1…ADR-24) · [docs/immich.md](docs/immich.md) (интеграция с Immich: API, варианты, шаг 1 — админ-настройки) · [docs/server.md](docs/server.md) (nginx, SSL, сервер, диагностика).
...
- **Источником истины является Git-репозиторий, а не локальные файлы `/memories/`**: важные факты, ограничения и инструкции должны храниться в версияции проекта, чтобы любой клон содержал тот же харнесс.
- **Папка `.github/harness/` — часть харнесс и структуры проекта**: в ней живут репозиторные заметки, которые должны быть сохранены вместе с кодом и переноситься между машинами.
- **ЖЁСТКОЕ ПРАВИЛО: локальная память репозитория (`/memories/repo/`) НЕ используется и НЕ ведётся.**
  Все факты, ограничения, грабли и решения фиксируются **только** в версионируемой документации
  репозитория: `AGENTS.md`, `docs/`, `.github/skills/*/SKILL.md`, `.github/harness/repo-memory.md`.
  Локальную память не читать как источник, не пополнять и не «синхронизировать» с харнессом — любой
  новый/изменённый контекст записывать сразу в проектную документацию. (Прежние записи
  `/memories/repo/` очищены 2026-08-18; важное перенесено в доки/харнесс.)
- **Если правило, отладка, миграция или исключение важно для повторения контекста — оно должно быть зафиксировано в репозитории**. Иначе новое устройство теряет контекст, а предыдущая сессия не воспроизводима.
...
9. **Многошаговые задачи.** При сериях команд со ссылками на пункты («выполни пункт N»,
   ...
   - **Проактивное управление контекстом:** при длинной сессии (сотни ходов) или подозрении на потерю
     ...
     «продавливать»: перечитать авторитетные источники (`AGENTS.md`, `docs/specification-*.md`, ключевые
...
10. **UI-итерации и позиционирование.** При правках отступов/выравнивания/порядка/размеров элементов:
    ...
    - **новые UI-элементы — в стиле существующих:** размеры иконок/кнопок/переключателей брать из
      существующих компонентов (`components/icons.tsx`, `styles/tokens.css`, соседние элементы), а не из
      источника ассета; новый элемент должен соответствовать остальным по размеру/виду (иначе повторные
      круги «иконка неправильного размера», «переключатель не как в приложении»);
    ...
    - **мобильная проверка:** интегрированный браузер — desktop-only; если правка касается мобильного
      макета/поведения (в т.ч. pdf.js, прогресс-бары при медленной сети) — проверить на узком вьюпорте
      (resize) и явно предупредить пользователя, что десктопная проверка не покрывает реальное
      устройство (Samsung WebView и т.п.) — попросить подтвердить на устройстве;
