...
- **Каркас харнесса для нового проекта — навык `.github/skills/harness-init/`** (процедура + шаблоны `references/*`: `AGENTS.md`, `specification.md`, модульная спецификация, ADR, `SKILL.md`, `*.agent.md`, `repo-memory.md`, `README`/`.env.example`) и промпт-обёртка `.github/prompts/init-harness.prompt.md` (её копия лежит в профиле пользователя и доступна в произвольном проекте).
...
5. **`app.listen` гейтится** в `app.ts`: слушать при `NODE_ENV=production` ЛИБО прямом запуске. Под pm2 `process.argv[1]` — враппер pm2 (не скрипт) → argv-проверка даёт `false`; основной сигнал — `NODE_ENV=production` (ставит деплой-скрипт). В dev Vite монтирует `app` сам — слушать нельзя.
   - Импорт PDF (этап 3): python-скрипт (`extract_pdf.py`) на Windows пишет в stdout в консольную кодировку (cp1251) — принудительно `sys.stdout.reconfigure(encoding='utf-8')`, иначе Node читает «кракозябры». JS `\b` не считает кириллицу word-char: `/\bитого\b/i` не матчит «Итого», а просто `итого` матчит как ПОДСТРОКУ — границы слова задавать явно (`/(?:^|\s)итого(?=[\s,.:;-]|$)/i`). Грабля: детектор «Итого» в `draft.ts` (`итого[^\d]*…`) матчил «итого» внутри слова «сшитого» → преждевременно включался режим «после Итого» и терялись имена последующих позиций (заказ материалов №7: 45 позиций с именем «позиция N»); аналогично в `parseItemTables` был `includes('итого')` — там проверка по наименованию `/(?:^|\s)итого(?=[\s,:;]|$)/i`. Та же грабля в `normalizeName` (addendum.ts): `\bтолщиной\b` не удаляет «толщиной» — границы слова задавать пробелами/началом/концом строки (`/(?:^|\s)(?:толщиной|слоем)(?=\s|$)/`), иначе «…раствором | до 25мм» (смета) ≠ «…раствором | толщиной до 25мм» (акт) → строка уходит в «added» отчёта «Ход работ». Грабля «номер в дефисах»: в PDF подрядчика номер позиции может быть обёрнут дефисами («-7-», «-8-») — regex `^(\d+)…`/`ITEM_START_RE` (`^\d+\.?\s+`) не матчат строку с ведущим дефисом, и такие позиции теряются (`parseItemText`); допускать `-?(\d+)-?…` в `SINGLE_LINE_ITEM`/`CONTINUATION_ROW`/`ITEM_START_RE`. И отдельно: `addendum` должен входить в `ITEM_TYPES` (`buildDraft`), иначе позиции доп. соглашения вообще не разбираются. Грабля «дата ведомости»: для ведомостей `classify.ts` искал самую позднюю дату только в шапке текста (`headerDates(text, 700)`), а у «длинных» ведомостей последние строки выходят за 700 символов → дата документа и имя PDF (`settlement_<type>_<date>.pdf`) оставались прежними («…26.08.2026» при строках до 16.09.2026), и повторный импорт обновлённой ведомости бился в идемпотентность тип+дата (409). Дата уточняется в `buildDraft` по разобранным строкам (`lastRowDate`; сравнивать по ISO, а не лексикографически по `dd.mm.yyyy` — иначе максимум = «31.07»). И: единственное число строки-подсуммы «Подотчётные прораба» попадает то в `balance`, то в `used` → `foremenAmount` в `overview.ts` учитывает обе колонки (`Math.abs(r.balance ?? r.used ?? 0)`), иначе «Учтено по актам» не вычитает подотчётные и в UI висит ложная сноска о расхождении на сумму подотчёта (50 000 ₽).
...
15. **Проверка ценовой политики перед анализом (см. `docs/pricing.md`).** Если выбрана
    ...
    окно peak/off-peak
    (пиковые часы — **01:00–04:00 и 06:00–10:00 UTC по будням (понедельник–пятница)**, все остальные — off-peak; полные
