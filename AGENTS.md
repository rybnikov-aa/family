...
- **`sharp` (превью изображений «Дневника») на сервере: `Unsupported CPU: Prebuilt binaries for Linux x64 require v2 microarchitecture`** — CPU сервера (QEMU 2.5+, x86-64-v1, без SSE4.2/POPCNT/AVX) не поддерживает пребилты sharp ≥0.34. Версия **зафиксирована `~0.33.5`** (`backend/package.json`, libvips 8.15, v1-совместима; проверено на сервере). **Не апгрейдить sharp** на серверный VPS без проверки `lscpu`/`node -e "require('sharp')"`. Симптом поломки: бэкенд падает при старте → 502 на весь API (в т.ч. вход). Быстрый фикс без пересборки (бандл экстернализирует sharp): `cd $SERVER && npm install --omit=dev sharp@~0.33.5 && pm2 restart family-backend`.
