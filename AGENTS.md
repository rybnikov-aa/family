...
1. **Документация синхронно с кодом.** При изменении кода (требования, конфиг, API, UI) обновлять **все**: `docs/specification.md` (первой — спецификация), затем `docs/server.md`, `README.md` и соответствующие `.env.example`. Код корректен, если удовлетворяет критериям приёмки из спецификации.
