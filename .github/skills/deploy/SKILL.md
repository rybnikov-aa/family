...
1. Убедиться, что в корневом `.env` корректная конфигурация `DEPLOY_*` (или переменные окружения).
...
- `server/data/` (SQLite-база VPS).
- `public_html/.well-known` и прочие подпапки фронтенда вне репозитория.
- Подпапки проектов, которых нет в репозитории (не удаляются). Папки проектов из репозитория перед перезаписью бэкапятся в `/tmp/family-projects-backup-<ts>`.
