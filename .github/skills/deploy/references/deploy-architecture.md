...
4. **Проекты** → `$PUBLIC`: существующие подпапки из репозитория бэкапятся в `/tmp/family-projects-backup-<ts>` перед перезаписью; подпапки, которых нет в репо, на сервере не удаляются.
