...
4. **Проекты** → `$PUBLIC`: существующие подпапки из репозитория бэкапятся в `/tmp/family-projects-backup-<ts>` перед перезаписью; подпапки, которых нет в репо, на сервере не удаляются. Подпапки проектов (`renovation/` и т.п.) копируются в корень `public_html/<slug>/`; общие файлы (`styles.css`, `theme.js`, `icon-sprite.svg`) — в `public_html/projects/` (страницы ссылаются на них как `/projects/...`, см. `docs/server.md`).
