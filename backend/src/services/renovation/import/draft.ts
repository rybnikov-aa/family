...
/** Типы документов, у которых разбираются позиции (акт работ / заказ материалов / доп. соглашение). */
const ITEM_TYPES = new Set(['work_act', 'material_order', 'addendum']);
...
// Номер позиции в PDF шаблона подрядчика может быть обёрнут дефисами
// («-7- Монтаж редуктора давления …», «-8- …») — допускаем ведущий/хвостовой дефис.
...
/** Начало строки позиции (номер, возможно обёрнутый дефисами: «1 …», «-7- …»). */
