...
заменяет на тестовом сервере базы из основного сервера, запускает sanity-тесты и только при их
успехе публикует тот же build на `my.rybnikov.su`. При ошибке этапа основной сервер не меняется.
...
завершают тесты с ненулевым кодом. В pipeline это блокирует публикацию на основной сервер.
