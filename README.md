...
- **Раздел «Дневник»** — события семьи в отдельной БД `backend/data/diary.sqlite` (путь — `DIARY_DB_PATH`): блоки с фотографиями, названием, датой (или периодом дат в pill) и кратким описанием. Пользователь переключает макет кнопками с иконками: «список» (по умолчанию, на всю ширину) и «карточки» (сетка на 3 столбца). События отсортированы по дате начала (свежие — раньше). Клик по блоку открывает страницу события (`#/diary/:id`) с полным содержимым (обложка, markdown-описание, фотографии на местах маркеров и галерея оставшихся файлов). Подробное описание редактируется на отдельной фокусной странице `#/diary/:id/edit` (admin): markdown слева, живой предпросмотр с фотографиями справа (mobile — стеком), вставка фото из события миниатюрами и загрузка новых («Добавить фото») с вставкой у курсора; «Сохранить» пишет контент сразу на сервер — двухступенчатого сохранения («описание → событие») нет, черновик автосохраняется в localStorage с подтверждением при уходе. Добавление (кнопка «+») и редактирование («карандашик») — только admin: форма с загрузкой нескольких фотографий, выбором основной, периодом дат и компактным черновиком описания. На карточке admin также доступны кнопки-иконки: «Редактировать событие», «Редактировать описание», «Редактировать фотографии» (модалка «Фотографии» с мгновенным сохранением через PATCH) и «Удалить». Изображения сохраняются на сервере в `DIARY_IMAGES_DIR` (по умолчанию `images`; dev — `backend/images/`, сервер — `server/images/`) в уникальной подпапке `images/<folder>/` на каждое событие; каталог сохраняется при деплое (как `data/` и `docs/`). Раздача — `GET /api/diary/images/:folder/:file` (под авторизацией): в карточках/галерее — уменьшенное превью (WebP, `?preview=1`, генерируется `sharp` в подпапке `thumbs/`), полный размер в максимальном качестве — только при открытии изображения на весь экран.
...
| `docs/adr.md`                      | Решения и обоснования (ADR-1…ADR-25): альтернативы, обоснования                                                                                      |
...
| `docs/specification-diary.md`      | Спецификация модуля «Дневник» (события семьи, отдельная БД + изображения, редакторы)                                                                 |
...
| `docs/frontend-design.md`          | Дизайн-система фронтенда: токены, шкалы, примитивы, статус фаз                                                                                       |
| `docs/pricing.md`                  | Ценовая политика DeepSeek API (peak/off-peak) для выбора модели                                                                                      |
| `docs/server.md`                   | Справочник по серверу: пути, nginx, SSL, деплой, диагностика                                                                                         |
| `docs/backup.md`                   | Справочник по бэкапу и восстановлению: команды backup/restore, конфигурация `BACKUP_*`/`RESTORE_*`, cron, восстановление на новый VPS                |
...
публикация напрямую на `my.rybnikov.su` (`npm run deploy -- --no-pdf-setup`). Тестовый сервер при
этом остаётся несинхронизированным — после его возвращения прогнать `npm run pipeline` для
синхронизации версии.
...
Восстановление на новый VPS (после провижининга nginx/SSL/node — чек-лист в `docs/backup.md`):
...
Подробности и конфигурация (`BACKUP_*`/`RESTORE_*`) — в `docs/backup.md` и `.env.example`.
...
> `--no-pdf-setup` на новых хостах обязателен: из-за грабли `HOME` (см. docs/server.md §1.4) pdf-setup создаёт venv по битому пути; venv ставится вручную.
