...
Портал закрыт авторизацией: учётные записи хранятся в SQLite (таблицы `users` + `sessions`), пароли — хэши scrypt (без новых зависимостей, `node:crypto`). Роли: `admin` (управление VPS, загрузка PDF, управление пользователями) и `user` (чтение). Отображаемое имя и пароль своей учётки пользователь меняет на странице «Профиль» (`PATCH /api/auth/profile`; смена пароля — с подтверждением текущим).
